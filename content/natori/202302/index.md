...
{{< addbib label="fs09" title="Flajolet, Philippe; Sedgewick, Robert. Analytic combinatorics. Cambridge: Cambridge University Press (2009)." >}}
...
です。このとき、$f(xq)=\frac{1+x^{-1}q^{-1}}{1+xq}f(x)=x^{-1}q^{-1}f(x)$ なので

$$
\sum_{n\in\mathbb{Z}}c_nx^nq^n=\sum_{n\in\mathbb{Z}}c_nx^{n-1}q^{-1}
$$

となります。$x^n$ の係数を比較して、$c_{n+1}=q^{n+1}c_n$ を得ます。$1+2+\cdots+n=\frac{n(n+1)}{2}$ なので、$c_n=q^{n(n+1)/2}c_0$ となります。あとは $c_0$ を求めればよいです。$c_0$ は $f(x)$ の定数項なので、展開することで
...
$$
c_0=\sum_{\lambda:\text{ヤング図形}}q^{|\lambda|}=\sum_{n=0}^{\infty}p(n)q^n
$$

となることがわかりました。ここで $p(n)$ はサイズ $n$ のヤング図形の個数、すなわち分割数です。よく知られているように分割数の母関数は
...
この定理はヤコビの三重積公式から証明することができます。新しい変数 $z$ を用意し、ヤコビの三重積公式に $q=z^3, x=-z^{-1}$ を代入することで、左辺は
...
分割数 $p(n)$ の母関数はオイラーの五角数定理の左辺の逆数であることから

$$
\sum_{n=0}^{\infty}p(n)x^n=\frac{1}{\prod_{i}(1-x^i)}=\frac{1}{\sum_{m\in\mathbb{Z}}(-1)^mx^{m(3m+1)/2}}
$$

が得られます。この式から分割数の漸化式が得られるので、$p(1),p(2),\ldots,p(N)$ を時間計算量 $O(N\sqrt{N})$ で計算できます。

さらに形式的冪級数のテクニックを用いると $O(N\log N)$ に高速化することもできます。
